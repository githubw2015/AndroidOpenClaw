...
**设备路径**: `/sdcard/AndroidOpenClaw/openclaw.json`
...
```json
{
  ...
  "workspacePath": "/sdcard/AndroidOpenClaw/workspace/skills", // 用户技能
  "managedPath": "/sdcard/AndroidOpenClaw/.skills",      // 管理技能
...
```bash
# 创建配置目录
adb shell mkdir -p /sdcard/AndroidOpenClaw/config

# 推送配置文件
adb push openclaw.json /sdcard/AndroidOpenClaw/config/
```
...
⚠️ **路径说明**:
- `/sdcard/AndroidOpenClaw/`: 应用外部存储根目录（用户可访问）
- `/sdcard/AndroidOpenClaw/openclaw.json`: 主配置文件（从 assets/openclaw.json.default.txt 复制）
- `/sdcard/AndroidOpenClaw/workspace/`: 工作区目录（sessions、skills、memory 等）
