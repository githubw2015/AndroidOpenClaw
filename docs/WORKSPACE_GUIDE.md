# AndroidOpenClaw Workspace 使用指南

## 概述

AndroidOpenClaw workspace 对齐 OpenClaw 架构，为用户提供可直接访问和编辑的工作区。

## 🎯 Workspace 设计理念

### OpenClaw vs AndroidOpenClaw

| 特性 | OpenClaw | AndroidOpenClaw |
|------|----------|----------------|
| **Workspace 路径** | `~/.openclaw/workspace/` | `/sdcard/AndroidOpenClaw/workspace/` |
| **Skills 路径** | `~/.openclaw/workspace/skills/` | `/sdcard/AndroidOpenClaw/workspace/skills/` |
| **访问方式** | 文件系统直接访问 | 文件管理器直接访问 |
| **权限要求** | 文件系统权限 | MANAGE_EXTERNAL_STORAGE |

### 为什么选择 `/sdcard/AndroidOpenClaw/workspace/`？
...
```
/sdcard/
├── androidopenclaw-workspace/          ← 用户工作区（对标 ~/.openclaw/workspace/）
│   ├── skills/                        ← 用户自定义 Skills
│   │   ├── my-custom-skill/
│   │   │   └── SKILL.md
│   │   ├── wechat-automation/
...
1. Workspace Skills     /sdcard/AndroidOpenClaw/workspace/skills/     ← 用户自定义（最高优先级）
2. Managed Skills       /sdcard/AndroidOpenClaw/.skills/              ← 包管理器安装
...
2. 导航到 `/sdcard/AndroidOpenClaw/workspace/skills/`
3. 创建新目录，如 `my-skill/`
...
```bash
# 目录结构
/sdcard/AndroidOpenClaw/workspace/skills/wechat-automation/
└── SKILL.md
```
...
**WeChat Automation** - AndroidOpenClaw Custom Skill 💬
```
...
2. 完全自定义内容
3. AndroidOpenClaw 会优先加载 workspace 版本

**示例**: 覆盖 `app-testing` skill

```bash
# 创建目录
/sdcard/AndroidOpenClaw/workspace/skills/app-testing/
└── SKILL.md  # 你的自定义版本
```

### 3. 热重载（Hot Reload）

AndroidOpenClaw 支持 workspace skills 的热重载：
...
```bash
# 使用 adb 备份
adb pull /sdcard/AndroidOpenClaw/workspace/ ./backup/

# 或使用文件管理器
# 复制整个 androidopenclaw-workspace 目录到电脑
```
...
```bash
# 使用 adb 恢复
adb push ./backup/androidopenclaw-workspace/ /sdcard/
...
cd /sdcard/AndroidOpenClaw/workspace/
git init
...
```
/sdcard/AndroidOpenClaw/workspace/skills/complex-skill/
├── SKILL.md          ← 主文件（必需）
├── scripts/
...
**A**: AndroidOpenClaw 不会自动创建 workspace 目录。你需要手动创建：

```bash
# 使用 adb
adb shell mkdir -p /sdcard/AndroidOpenClaw/workspace/skills/

# 或使用文件管理器
# 导航到 /sdcard/
# 创建目录 androidopenclaw-workspace/skills/
```
...
1. 目录结构正确？`/sdcard/AndroidOpenClaw/workspace/skills/my-skill/SKILL.md`
...
**A**: 目前不支持嵌套子目录。所有 skills 必须在 `/sdcard/AndroidOpenClaw/workspace/skills/` 的第一级。
...
❌ 错误:
/sdcard/AndroidOpenClaw/workspace/skills/category/my-skill/SKILL.md
```
...
AndroidOpenClaw workspace 完全对齐 OpenClaw 架构，为用户提供：
...
**AndroidOpenClaw Workspace** - Aligned with OpenClaw Architecture 🤖📱
