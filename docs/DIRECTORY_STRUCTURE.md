# AndroidOpenClaw 目录结构设计

## 对齐 OpenClaw 架构

参考 OpenClaw 的 `~/.openclaw/` 目录结构，AndroidOpenClaw 采用类似的组织方式。
...
|----------|----------------|------|
| `~/.openclaw/` | `/sdcard/AndroidOpenClaw/` | 主目录（隐藏目录） |
| `~/.openclaw/workspace/` | `/sdcard/AndroidOpenClaw/workspace/` | 用户工作区（可见） |
| `~/.openclaw/skills/` | `/sdcard/AndroidOpenClaw/skills/` | 托管 Skills |
| `~/.openclaw/agents/` | `/sdcard/AndroidOpenClaw/agents/` | Agent 数据 |
| `~/.openclaw/openclaw.json` | `/sdcard/AndroidOpenClaw/openclaw.json` | 主配置 |
...
/sdcard/
│
├── .androidopenclaw/                      ← 主目录（隐藏，类似 ~/.openclaw/）
│   ├── openclaw.json                     ← 主配置文件
...
│       └── index.html
│
└── androidopenclaw-workspace/             ← 用户工作区（可见目录）
    ├── skills/                           ← 用户自定义 Skills
    │   ├── my-custom-skill/
    │   │   └── SKILL.md
    │   └── wechat-automation/
...
**隐藏目录** (`.androidopenclaw/`):
- 系统配置和数据
- 日志文件
- 自动生成的内容
- 用户通常不需要直接编辑

**可见目录** (`androidopenclaw-workspace/`):
- 用户自定义 skills
- 个性化配置（IDENTITY.md 等）
- 会话工作目录
- 用户经常访问和编辑
...
1. androidopenclaw-workspace/skills/     ← 用户自定义（最高）
2. .androidopenclaw/skills/              ← 托管 Skills
...
```json
{
  ...
  "agents": {
    "defaults": {
      "workspace": "/sdcard/androidopenclaw-workspace",
      "model": {
        "provider": "anthropic",
        "id": "claude-opus-4-6"
      }
...
```
/sdcard/AndroidOpenClaw/
├── openclaw.json              ← 移动自 AndroidOpenClaw/config/
├── config/
│   └── models.json            ← 保留作为可选配置
└── ...

/sdcard/AndroidOpenClaw/workspace/
├── skills/                    ← 移动自 AndroidOpenClaw/workspace/skills/
├── IDENTITY.md                ← 新增
├── BOOTSTRAP.md               ← 新增
└── ...
```
...
- **当前实现**: `app/src/main/java/com/xiaomo/androidopenclaw/agent/skills/SkillsLoader.kt`
