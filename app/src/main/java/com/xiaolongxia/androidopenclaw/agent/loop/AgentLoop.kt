package com.xiaolongxia.androidopenclaw.agent.loop
 ...
 *
 * AndroidOpenClaw adaptation: iterative agent loop, tool calling, progress updates.
 * Note: OpenClaw splits the loop across run.ts (retry/overflow) and subscribe.ts (streaming/tool dispatch);
 * AndroidOpenClaw merges both into this single class with non-streaming batch calls.
 */


import com.xiaolongxia.androidopenclaw.logging.Log
import com.xiaolongxia.androidopenclaw.agent.context.ContextErrors
import com.xiaolongxia.androidopenclaw.agent.context.ContextManager
import com.xiaolongxia.androidopenclaw.agent.context.ContextRecoveryResult
import com.xiaolongxia.androidopenclaw.agent.context.ContextWindowGuard
import com.xiaolongxia.androidopenclaw.agent.context.ToolResultContextGuard
import com.xiaolongxia.androidopenclaw.agent.session.HistorySanitizer
...
import com.xiaolongxia.androidopenclaw.agent.tools.ToolCallDispatcher
...
import com.xiaolongxia.androidopenclaw.providers.llm.ToolCall
import com.xiaolongxia.androidopenclaw.providers.llm.systemMessage
import com.xiaolongxia.androidopenclaw.providers.llm.userMessage
import com.xiaolongxia.androidopenclaw.providers.llm.assistantMessage
import com.xiaolongxia.androidopenclaw.providers.llm.toolMessage
import com.xiaolongxia.androidopenclaw.util.LayoutExceptionLogger
...
class AgentLoop(
    ...
    private val logDir = File("/sdcard/AndroidOpenClaw/workspace/logs")
    ...
    private suspend fun runInternal(
                    ...
                    messages.add(
                        assistantMessage(
                            ...
                            toolCalls = response.toolCalls.map {
                                com.xiaolongxia.androidopenclaw.providers.llm.ToolCall(
                                    id = it.id,
                                    name = it.name,
                ...
                if (isContextOverflow && contextManager != null) {
                    ...
                    if (e.message?.contains("timeout", ignoreCase = true) == true) {
                        ...
                        finalContent = buildString {
                            ...
                            // Error type
                            when (e) {
                                is com.xiaolongxia.androidopenclaw.providers.LLMException -> {
                                    append("**错误类型**: API 调用失败\n")
                                    append("**错误信息**: ${e.message}\n\n")
                                    append("**建议**: 请检查模型配置和 API Key 是否正确\n")
                                    append("**配置文件**: /sdcard/AndroidOpenClaw/openclaw.json\n")
                                }
                                else -> {
                                    append("**错误信息**: ${e.message}\n")
                                }
        ...
        if (effectiveFinalContent.isNotEmpty()) {
            messages.add(com.xiaolongxia.androidopenclaw.providers.llm.Message(
                role = "assistant",
