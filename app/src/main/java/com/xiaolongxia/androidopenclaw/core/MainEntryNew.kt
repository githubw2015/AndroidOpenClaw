...
import com.xiaolongxia.androidopenclaw.ext.mmkv
import com.xiaolongxia.androidopenclaw.ext.simpleSafeLaunch
...
import com.xiaolongxia.androidopenclaw.BuildConfig
...
object MainEntryNew {
    ...
    const val ACTION_AGENT_PROGRESS = "com.xiaolongxia.androidopenclaw.ACTION_AGENT_PROGRESS"
    ...
    fun initialize(app: Application) {
        ...
        try {
            ...
            val llmProvider = com.xiaolongxia.androidopenclaw.providers.UnifiedLLMProvider(application)
            ...
            val workspacePath = "/sdcard/AndroidOpenClaw/workspace"
            ...
            val memoryManager = com.xiaolongxia.androidopenclaw.agent.memory.MemoryManager(
            ...
            val workspaceDir = com.xiaolongxia.androidopenclaw.workspace.StoragePaths.workspace.also {
                it.mkdirs()
    ...
    fun runWithSession(
        ...
        val allMessages = if (dmHistoryLimit != null && dmHistoryLimit > 0) {
            ...
            com.xiaolongxia.androidopenclaw.agent.session.HistorySanitizer
        ...
        job = scope.simpleSafeLaunch(
            {
                ...
                com.xiaolongxia.androidopenclaw.gateway.GatewayServer.broadcastChatMessage(
                ...
                val cleanFinalContent = com.xiaolongxia.androidopenclaw.util.ReplyTagFilter.strip(
                ...
                if (cleanFinalContent.isNotEmpty()) {
                    ...
                    com.xiaolongxia.androidopenclaw.ui.float.SessionFloatWindow.updateLatestMessage(cleanFinalContent)
            ...
            { exception ->
                ...
                val errorMessage = buildString {
                    ...
                    // 如果是 LLM 异常，添加更详细的信息
                    if (exception is com.xiaolongxia.androidopenclaw.providers.LLMException) {
                        append("**类型**: API 调用失败\n")
                ...
                // 保存错误到 session
                try {
                    session.addMessage(com.xiaolongxia.androidopenclaw.providers.LegacyMessage(
                        role = "assistant",
